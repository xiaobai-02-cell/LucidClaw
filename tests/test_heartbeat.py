...
class TestHeartbeatPacemaker(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # 保存原始 TASKS_FILE 路径
        import lucidclaw.core.config
        self.original_tasks_file = lucidclaw.core.config.TASKS_FILE
        
        # 设置临时任务文件
        lucidclaw.core.config.TASKS_FILE = self.temp_file.name
        
        # 同时 patch heartbeat 模块中的引用
        import lucidclaw.core.heartbeat
        lucidclaw.core.heartbeat.TASKS_FILE = self.temp_file.name
    ...
    def tearDown(self):
        ...
        # 恢复原始路径
        import lucidclaw.core.config
        lucidclaw.core.config.TASKS_FILE = self.original_tasks_file
        
        import lucidclaw.core.heartbeat
        lucidclaw.core.heartbeat.TASKS_FILE = self.original_tasks_file
    ...
    def test_no_tasks_file(self):
        ...
        async def run_test():
            # 直接测试逻辑，不实际等待
            import lucidclaw.core.heartbeat as hb
            # 模拟 TASKS_FILE 不存在

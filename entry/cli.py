...
from lucidclaw.core.provider import get_provider
...
app = typer.Typer(help="LucidClaw - 极客专属的赛博智能终端")
console = Console()
...
def config_wizard():
    console.clear()
    console.print(Panel(
        "👾 Welcome to [bold #8d52ff]LucidClaw[/bold #8d52ff]...\n\n☁️[dim] 请完成模型配置，我们将把密钥安全固化在本地。[/dim]", 
        title="[bold white]✦  LucidClaw Config[/bold white]", 
        border_style="#8d52ff"
    ))
    ...
    if not provider_raw:
        console.print("[dim #8d52ff]✦   录入中断，LucidClaw 配置已取消。[/dim #8d52ff]")
        return
    ...
    console.print(Panel(
        ...
        f"👉 输入 [bold #00ffff]lucidclaw run[/bold #00ffff] 即可启动系统！",
        border_style="#00ffff"
    ))

def _show_boot_error():
    console.print(Panel(
        "[bold #00ffff]LucidClaw未完成配置![/bold #00ffff]\n\n"
        "[#8d52ff]检测到 API Key、模型或Baseurl。请重新执行以下命令完成配置：[/#8d52ff]\n"
        "[bold #00ffff]lucidclaw config[/bold #00ffff]",
...
def run_agent():
    ...
    import entry.main as lucidclaw_main
    lucidclaw_main.main()
...
def run_monitor():    
    ...
    try:
        import entry.monitor as lucidclaw_monitor
        lucidclaw_monitor.main()

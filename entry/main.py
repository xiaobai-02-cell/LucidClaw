...
{RESET}"""
    ...
    sub_title = f"{WHITE}{BOLD} 👾 Welcome to the {PURPLE}{BOLD}LucidClaw{RESET}{WHITE}{BOLD} !  {RESET}"
    ...
    tip = (
        f"{PURPLE} ✦ {RESET}"
        f"{SILVER}{PURPLE}{BOLD}LucidClaw{RESET} 已完成启动。输入命令开始，输入 {PURPLE}/exit{RESET}{SILVER} 退出。{RESET}\n"
    )
        ...
        async def user_input_loop():
            ...
            while True:
                try:
                    ...
                    if user_input.lower() in ["/exit", "/quit"]:
                        cprint("  \033[38;5;141m✦ 记忆已固化，LucidClaw 进入休眠。\033[0m")
                        break
                        
                except (KeyboardInterrupt, EOFError):
                    cprint("\n  \033[38;5;141m✦ 强制中断，LucidClaw 进入休眠。\033[0m")
